...
bool save_time(char str[], int num_blocks, float fir_time, float fft_time, float mem_time_in, float mem_time_out, const int nChannels, const int nTaps){
	...
	//------------------
		FILEOUT << std::fixed << std::setprecision(8) << num_blocks << "\t" << fir_time/1000.0 << "\t" << (fir_time+fft_time)/1000.0 << "\t" << std::scientific << bandwidth << "\t" << flops << "\t" << std::fixed << mem_time_in/1000.0  << "\t" << mem_time_out/1000.0  << endl;
	FILEOUT.close();
	return 0;
}
