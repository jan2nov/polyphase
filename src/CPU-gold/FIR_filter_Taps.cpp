...
int main(void) {
	...
	PwI3_FILE.open("pwi3_T_perf.dat");PwI4_FILE.open("pwi4_T_perf.dat");
	SwI4_FILE.open("swi4_T_perf.dat");PwI6_FILE.open("pwi6_T_perf.dat");
	
	int data[5]={8,12,16,20,24};

	for(int data_counter=0;data_counter<5;data_counter++){
		nTaps=data[data_counter];
		DATA_BLOCKS=2500;
		...
		cout << "-------------> Number of Taps: " << nTaps << endl;
