...
int main(void) {
	...
	ofstream PwI3_FILE,PwI4_FILE,SwI4_FILE,S1wI4_FILE,PwI6_FILE;
	PwI3_FILE.open("pwi3_perf.dat");
	PwI4_FILE.open("pwi4_perf.dat");
	SwI4_FILE.open("swi4_perf.dat");
	S1wI4_FILE.open("s1wi4_perf.dat");
	PwI6_FILE.open("pwi6_perf.dat");
	
	int data[5]={625,1250,2500,5000,10000};
	...
	for(int data_counter=0;data_counter<5;data_counter++){
		...
		PwI3_FILE << data[data_counter]*24 << " " << mean_FIR << " " << mean_W << " " << bandwidth << " " << flops << " " << mean_FFT << endl;
		...
		bandwidth=((MEMORY_READ+MEMORY_WRITE)/mean_FIR)*4.0;
		flops=(FLOPS/mean_FIR);
		SwI4_FILE << data[data_counter]*24 << " " << mean_FIR << " " << mean_W << " " << bandwidth << " " << flops << " " << mean_FFT << endl;
		...
		bandwidth=((MEMORY_READ+MEMORY_WRITE)/mean_FIR)*4.0;
		flops=(FLOPS/mean_FIR);
		S1wI4_FILE << data[data_counter]*24 << " " << mean_FIR << " " << mean_W << " " << bandwidth << " " << flops << " " << mean_FFT << endl;
		...
		bandwidth=((MEMORY_READ+MEMORY_WRITE)/mean_FIR)*4.0;
		flops=(FLOPS/mean_FIR);
		PwI4_FILE << data[data_counter]*24 << " " << mean_FIR << " " << mean_W << " " << bandwidth << " " << flops << " " << mean_FFT << endl;		
		...
		bandwidth=((MEMORY_READ+MEMORY_WRITE)/mean_FIR)*4.0;
		flops=(FLOPS/mean_FIR);
		PwI6_FILE << data[data_counter]*24 << " " << mean_FIR << " " << mean_W << " " << bandwidth << " " << flops << " " << mean_FFT << endl;
	...
	PwI3_FILE.close();PwI4_FILE.close();SwI4_FILE.close();S1wI4_FILE.close();PwI6_FILE.close();
	
	
	

	sprintf(str,"pwi3_thread_perf_t.dat");PwI3_FILE.open(str);
	sprintf(str,"pwi4_thread_perf_t.dat");PwI4_FILE.open(str);
	sprintf(str,"pwi6_thread_perf.dat");PwI6_FILE.open(str);
	for(int th_counter=1;th_counter<25;th_counter++){
		
		THREADS=th_counter*2;
		cout << "--------> THREADS=" << THREADS << endl;
		
		//**************
		// PARALLEL CODE WITH INTRINSIC Mk3
		//**************
		cout << "PARALLEL CODE WITH INTRINSIC Mk3" << endl;
		...
		bandwidth=((MEMORY_READ+MEMORY_WRITE)/mean_FIR);
		flops=(FLOPS/mean_FIR);
		PwI4_FILE << THREADS << " " << mean_FIR << " " << mean_W << " " << bandwidth << " " << flops << endl;
		...
		bandwidth=((MEMORY_READ+MEMORY_WRITE)/mean_FIR);
		flops=(FLOPS/mean_FIR);
		PwI6_FILE << THREADS << " " << mean_FIR << " " << mean_W << " " << bandwidth << " " << flops << endl;
		//********************************************************************************************
		//***************************** PERFORMANCE MEASUREMENTS *******************************
		//********************************************************************************************
	}
	PwI3_FILE.close();PwI4_FILE.close();PwI6_FILE.close();
